...
runcmd:
 ...
 - 'for p in `echo ${dynatrace_plugins}`; do rm -f $${p} && blobxfer-1.9.4 download --storage-account ${plugin_storage_account} --storage-account-key ${plugin_storage_key} --remote-path "plugin/custom.remote.python.snmp_palo_alto.zip" --local-path . && unzip -o $${p} -d /opt/dynatrace/remotepluginmodule/plugin_deployment; done'
